...
class HTTP2Session {
    ...
    request(path, token, body, timeout) {
...
class HTTPClient {
    ...
    async _request(requestInfo) {
        try {
            ...
            const response = this.http2Session != null
                ? await this.makeHTTP2Request(requestInfo.url.replace(this.origin, ''), this.applicationToken, requestInfo.data, requestInfo.timeout || DEFAULT_TIMEOUT)
                : await axiosAgent({
                    url: requestInfo.url,
    ...
    async makeHTTP2Request(path, token, body, timeout) {
